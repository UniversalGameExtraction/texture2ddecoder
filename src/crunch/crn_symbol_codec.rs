...
impl<'slice> symbol_codec<'slice>{
    ...
    pub fn decode_receive_static_data_model(&mut self, model: &mut static_huffman_data_model) -> bool{
        let total_used_syms = self.decode_bits(total_bits(cMaxSupportedSyms));
        ...
        for i in 0..num_codelength_codes_to_send as usize{
            dm.m_code_sizes[g_most_probable_codelength_codes[i] as usize] = self.decode_bits(3) as u8;
        }
        ...
        while ofs < total_used_syms {
            ...
            let code: u32 = self.decode(&dm);
            if code <= 16 {
                model.m_code_sizes[ofs as usize] = code as u8;
                ofs += 1;
            }else if code == cSmallZeroRunCode {
                let len = self.decode_bits(cSmallZeroRunExtraBits) + cMinSmallZeroRunSize;
                if len > num_remaining {
                    return false;
                }
                ofs += len;
            }else if code == cLargeZeroRunCode {
                let len = self.decode_bits(cLargeZeroRunExtraBits) + cMinLargeZeroRunSize;
                if len > num_remaining {
                    return false;
                }
                ofs += len;
                ...
                if code == cSmallRepeatCode {
                    len = self.decode_bits(cSmallNonZeroRunExtraBits) + cSmallMinNonZeroRunSize;
                }else{
                    len = self.decode_bits(cLargeNonZeroRunExtraBits) + cLargeMinNonZeroRunSize;
                }
    ...
    pub fn decode_bits(&mut self, num_bits: u32) -> u32{
        ...
        if num_bits > 16 as u32 {
            ...
            return ((a << 16) | b) as u32;
        }else{
            return self.get_bits(num_bits);
        }
    }
    pub fn decode(&mut self, model: &static_huffman_data_model) -> u32{
        ...
        if k <= pTables.m_table_max_code {
            ...
            len = t >> 16;
            assert!(model.m_code_sizes[sym as usize] as u32 == len);
        }else{
    ...
    fn get_bits(&mut self, num_bits: u32) -> u32{
        ...
        while self.m_bit_count < num_bits as i32 {
            ...
            self.m_bit_count += 8;
            assert!((self.m_bit_count <= cBitBufSize as i32));
