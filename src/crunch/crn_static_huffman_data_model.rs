...
pub struct decoder_tables{
    ...
    pub m_max_codes: [u32; (cMaxExpectedCodeSize + 1) as usize],
    pub m_val_ptrs: [i32;  (cMaxExpectedCodeSize + 1) as usize],
...
impl decoder_tables{
    pub fn init(&mut self, num_syms: u32, pCodesizes: &[u8], mut table_bits: u32) -> bool{
        let mut min_codes = [0 as u32; cMaxExpectedCodeSize as usize];
        if num_syms == (0 as u32) || table_bits > cMaxTableBits {
            return false;
        }
        self.m_num_syms = num_syms;
        let mut num_codes = [0 as u32; (cMaxExpectedCodeSize + 1) as usize];
        ...
        let mut sorted_positions = [0 as u32; (cMaxExpectedCodeSize + 1) as usize];
        let mut cur_code: u32 = 0;
        let mut total_used_syms: u32 = 0;
        let mut max_code_size: u32 = 0;
        let mut min_code_size: u32 = u32::MAX;
        for i in 1..=cMaxExpectedCodeSize as usize{
            let n = num_codes[i];
            if n == 0 {
                self.m_max_codes[i - 1] = 0;
            }else{
                min_code_size = core::cmp::min(min_code_size, i as u32);
                max_code_size = core::cmp::max(max_code_size, i as u32);
        ...
        if total_used_syms > self.m_cur_sorted_symbol_order_size {
            ...
            if is_power_of_2(total_used_syms as usize) == false {
                self.m_cur_sorted_symbol_order_size = core::cmp::min(num_syms, next_pow2(total_used_syms as usize) as u32);
            }
        ...
        for i in 0..num_syms as usize{
            let c: u32 = pCodesizes[i] as u32;
            if c != 0 {
                assert!(num_codes[c as usize] != 0);
        ...
        if table_bits != 0 {
            ...
            for codesize in 1..=table_bits{
                ...
                let max_code: u32 = self.get_unshifted_max_code(codesize);
                ...
                for code in min_code..=max_code{
                    ...
                    assert!(pCodesizes[sym_index as usize] as u32 == codesize);
                    for j in 0..fillnum{
        ...
        for i in 0..cMaxExpectedCodeSize as usize{
        ...
        // sentinels
        self.m_max_codes[cMaxExpectedCodeSize as usize] = u32::MAX;
        self.m_val_ptrs[cMaxExpectedCodeSize as usize] = 0xFFFFF;

        self.m_table_shift = 32 - self.m_table_bits;
        return true;
    }
    #[inline]
    fn get_unshifted_max_code(&mut self, len: u32) -> u32{
        assert!(len >= 1 && len <= cMaxExpectedCodeSize);
        let k: u32 = self.m_max_codes[(len - 1) as usize];
        if k == 0 {
            return u32::MAX;
        }
        return ((k - 1) >> (16 - len)) as u32;
    }
}

#[derive(Default)]
pub struct static_huffman_data_model{
    pub m_total_syms: u32,
    pub m_code_sizes: alloc::vec::Vec<u8>,
    pub m_pDecode_tables: decoder_tables
}

impl static_huffman_data_model{
    pub fn clear(&mut self){
        *self = static_huffman_data_model::default();
    }
    #[allow(dead_code)]
    pub fn init(&mut self, total_syms: u32, pCode_sizes: &[u8], mut code_size_limit: u32) -> bool{
        assert!(total_syms >= (1 as u32) && total_syms <= cMaxSupportedSyms && code_size_limit >= (1 as u32));
        code_size_limit = core::cmp::min(code_size_limit, cMaxExpectedCodeSize);
        ...
        for i in 0..total_syms as usize{
            ...
            min_code_size = core::cmp::min(min_code_size, s);
            max_code_size = core::cmp::max(max_code_size, s);
        }
        ...
        if self.m_pDecode_tables.init(self.m_total_syms, &self.m_code_sizes, table_bits) == false {
            return false;
        }
        return true;
    }
    pub fn compute_decoder_table_bits(&mut self) -> u32{
        ...
        if self.m_total_syms > 16 {
            decoder_table_bits = core::cmp::min(1 + ceil_log2i(self.m_total_syms), cMaxTableBits);
        }
        return decoder_table_bits;
    }
    pub fn prepare_decoder_tables(&mut self) -> bool{
        let total_syms = self.m_code_sizes.len();
        assert!(total_syms >= 1 && total_syms as u32 <= cMaxSupportedSyms);
        self.m_total_syms = total_syms as u32;
        let table_bits = self.compute_decoder_table_bits();
        return self.m_pDecode_tables.init(self.m_total_syms, &self.m_code_sizes, table_bits);
    }
}