...
impl<'slice> CrnUnpacker<'slice>{
    ...
    pub fn init_tables(&mut self) -> bool{
        ...
        res = self.codec.decode_receive_static_data_model(&mut self.reference_encoding_dm);
        if !res {
            return res;
        }
    ...
    pub fn decode_color_endpoints(&mut self) -> bool{
        ...
        let has_etc_color_blocks: bool =    self.p_header.format.cast_to_uint() == CrnFormat::Etc1 as u32 ||
    ...
    pub fn decode_color_selectors(&mut self) -> bool{
        ...
        if has_subblocks{
            self.color_selectors.resize((self.p_header.color_selectors.num.cast_to_uint() as usize) << 1, 0);
        }else{
            self.color_selectors.resize(self.p_header.color_selectors.num.cast_to_uint() as usize, 0);
        }
        let mut s: u32 = 0;
                ...
                for h in 0..4{
                    for w in 0..4{
                        if has_subblocks{
                            ...
                            self.color_selectors[i << 1] |= ((s0 >> 1 & 1) | (s0 & 1) << 16) << (t & 15);
                        }
                        let s1 = selector >> (h << 3 | w << 1);
                        if has_subblocks {
                            self.color_selectors[i << 1 | 1] |= ((s1 >> 1 & 1) | (s1 & 1) << 16) << (t & 15);
                        }else{
    ...
    pub fn decode_alpha_selectors(&mut self) -> bool{
        let mut res = self.codec.start_decoding(&self.p_data[self.p_header.alpha_selectors.ofs.cast_to_uint() as usize..], self.p_header.alpha_selectors.size.cast_to_uint());
        if !res {
            return res;
        }
        let mut dm = StaticHuffmanDataModel::default();
    ...
    pub fn decode_alpha_selectors_etc(&mut self) -> bool{
        ...
        self.alpha_selectors.resize((self.p_header.alpha_selectors.num.cast_to_uint() as usize) * 6 + 1, 0);
        ...
        while i < self.alpha_selectors.len() - 1{
            ...
            for p in 0..16{
                ...
                WRITE_OR_U8_INTO_U16_BUFFER!(self.alpha_selectors, data_pos + byte_offset as usize, (((s as u16) << (8 - bit_offset)) & 0xFF));
                if bit_offset < 3 {
                    WRITE_OR_U8_INTO_U16_BUFFER!(self.alpha_selectors, data_pos + (byte_offset as usize) - 1, (s >> bit_offset) as u16);
                }
                d += 9 * ((p as i32 & 3) - (p as i32 >> 2));
                byte_offset = d >> 3;
                bit_offset = d & 7;
                WRITE_OR_U8_INTO_U16_BUFFER!(self.alpha_selectors, data_pos + (byte_offset as usize) + 6, ((s as u16) << (8 - bit_offset)) & 0xFF);
                if bit_offset < 3 {
                    WRITE_OR_U8_INTO_U16_BUFFER!(self.alpha_selectors, data_pos + (byte_offset as usize) + 5, (s >> bit_offset) as u16);
                }
            }
            i += 6;
            data_pos += 12;
    ...
    pub fn decode_alpha_selectors_etcs(&mut self) -> bool {
        ...
        self.alpha_selectors.resize(((self.p_header.alpha_selectors.num.cast_to_uint() as usize) * 3) + 1, 0);
        let mut s_linear = [0_u8; 8];
        let mut i: usize = 0;
        while i < ((self.alpha_selectors.len() - 1) << 1){
            ...
            for p in 0..16{
                ...
                let byte_offset = d >> 3;
                let bit_offset = d & 7;
                WRITE_OR_U8_INTO_U16_BUFFER!(self.alpha_selectors, i + byte_offset as usize, (((s as u16) << (8 - bit_offset)) & 0xFF));
                if bit_offset < 3 {
                    WRITE_OR_U8_INTO_U16_BUFFER!(self.alpha_selectors, i + (byte_offset as usize) - 1, (s >> bit_offset) as u16);
                }
            }
            i += 6;
        }
    ...
    pub fn unpack_level_2(&mut self, p_src: &'slice [u8], src_size_in_bytes: u32, dst_size_in_bytes: u32, mut row_pitch_in_bytes: u32, level_index: u32) -> Result<alloc::vec::Vec<u8>, &'static str>{
        ...
        let unpack_res = match format {
            CrnFormat::Dxt1 |
            CrnFormat::Etc1s => self.unpack_dxt1(&mut ret, row_pitch_in_bytes, blocks_x, blocks_y),
            ...
            CrnFormat::Dxt5Agbr |
            CrnFormat::Dxt5XGxR |
            CrnFormat::Etc2as => self.unpack_dxt5(&mut ret, row_pitch_in_bytes, blocks_x, blocks_y),
            
            CrnFormat::Dxt5a => self.unpack_dxt5a(&mut ret, row_pitch_in_bytes, blocks_x, blocks_y),
            
            CrnFormat::DxnXy |
            CrnFormat::DxnYx => self.unpack_dxn(&mut ret, row_pitch_in_bytes, blocks_x, blocks_y),
            
            CrnFormat::Etc1 |
            CrnFormat::Etc2 => self.unpack_etc1(&mut ret, row_pitch_in_bytes, blocks_x, blocks_y),
    ...
    pub fn unpack_dxt1(&mut self, p_dst: &mut [u8], output_pitch_in_bytes: u32, output_width: u32, output_height: u32) -> Result<bool, &'static str>{
        ...
        if self.block_buffer.len() < width as usize{
            self.block_buffer.resize(width as usize, BlockBufferElement::default());
        }
        ...
        for f in 0..self.p_header.faces.cast_to_uint() as usize{
            ...
            for y in 0..height{
                ...
                for x in 0..width as usize{
                    ...
                    if visible {
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos, self.color_endpoints[color_endpoint_index]);
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos + 1, self.color_selectors[color_selector_index]);
                    }
                    data_pos += 2;
                }
            ...
            for y in 0..height{
                ...
                for x in 0..width as usize{
                    ...
                    if visible {
                        ...
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos, self.alpha_endpoints[alpha0_endpoint_index] as u32 | ((p_alpha0_selectors[0] as u32) << 16));
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos + 1, (p_alpha0_selectors[1] as u32) | ((p_alpha0_selectors[2] as u32) << 16));
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos + 2, self.color_endpoints[color_endpoint_index]);
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos + 3, self.color_selectors[color_selector_index]);
                    }
                    data_pos += 4;
                }
                data_pos += delta_pitch_in_dwords as usize;
            ...
            for y in 0..height{
                ...
                for x in 0..width as usize{
                    ...
                    if visible {
                        ...
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos, self.alpha_endpoints[alpha0_endpoint_index] as u32 | ((p_alpha0_selectors[0] as u32) << 16));
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos + 1, (p_alpha0_selectors[1] as u32) | ((p_alpha0_selectors[2] as u32) << 16));
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos + 2, self.alpha_endpoints[alpha1_endpoint_index] as u32 | ((p_alpha1_selectors[0] as u32) << 16));
    ...
    pub fn unpack_etc1(&mut self, p_dst: &mut [u8], output_pitch_in_bytes: u32, output_width: u32, output_height: u32) -> Result<bool, &'static str>{
        ...
        if self.block_buffer.len() < (width << 1) as usize{
            self.block_buffer.resize((width << 1) as usize, BlockBufferElement::default());
        }
        let mut color_endpoint_index: usize = 0;
        ...
        for f in 0..self.p_header.faces.cast_to_uint() as usize{
            ...
            for y in 0..height{
                ...
                for x in 0..width as usize{
                    ...
                    diagonal_color_endpoint_index = self.block_buffer[x << 1 | 1].color_endpoint_index as usize;
                    self.block_buffer[x << 1 | 1].color_endpoint_index = color_endpoint_index as u16;
                    ...
                    if visible {
                        ...
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos + 1, self.color_selectors[selector_index << 1 | (flip as usize)]);
                    }
                    data_pos += 2;
                }
                data_pos += delta_pitch_in_dwords as usize;
        ...
        for f in 0..self.p_header.faces.cast_to_uint() as usize{
            ...
            for y in 0..height{
                ...
                for x in 0..width as usize{
                    ...
                    endpoint_reference >>= 2;
                    let e0 = self.color_endpoints[color_endpoint_index].to_le_bytes();
                    let color_selector_index: usize = match self.codec.decode(&self.selector_delta_dm[0]) {
						Ok(s) => s,
						Err(_) => return Err("Failed to decode ETC2 Texture.")
					} as usize;
                    let alpha0_selector_index: usize = match self.codec.decode(&self.selector_delta_dm[1]) {
						Ok(s) => s,
						Err(_) => return Err("Failed to decode ETC2 Texture.")
					} as usize;
                    if endpoint_reference != 0 {
                    ...
                    let e1 = self.color_endpoints[color_endpoint_index].to_le_bytes();
                    diagonal_color_endpoint_index = self.block_buffer[x << 1 | 1].color_endpoint_index as usize;
                    diagonal_alpha0_endpoint_index = self.block_buffer[x << 1 | 1].alpha0_endpoint_index as usize;
                    self.block_buffer[x << 1 | 1].color_endpoint_index = color_endpoint_index as u16;
                    self.block_buffer[x << 1 | 1].alpha0_endpoint_index = alpha0_endpoint_index as u16;
                    if visible {
                        ...
                        WRITE_TO_INT_BUFFER!(p_dst, data_pos + 3, self.color_selectors[color_selector_index << 1 | (flip as usize)]);
                    }
                    data_pos += 4;
                }
                data_pos += delta_pitch_in_dwords as usize;
